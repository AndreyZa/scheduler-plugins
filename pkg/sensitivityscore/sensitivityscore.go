...
func (s *SensitivityScore) Score(
	...
	// чем выше произведение, тем больше риск интерференции. Net намеренно
	// НЕ входит ни в сумму, ни в знаменатель: метрика в score не подаётся
	// (нет честной шкалы [0,1] без калибровки под NIC — см. redis_source.go),
	// а держать её вес в знаменателе значило бы молча сжимать вклад
	// остальных осей: score никогда не опускался бы ниже 25 при равных
	// весах, а рост w.net «разбавлял» бы LLC/NUMA/IO. Вернуть Net сюда —
	// только вместе с нормализацией самой метрики в redis_source.go.
	...
	// "горячие".
	maxPossible := (w.LLC + w.NUMA + w.IO) * 100
